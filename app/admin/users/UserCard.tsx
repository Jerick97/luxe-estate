...
export function UserCard({ user, translations: t }: UserCardProps) {
  ...
  return (
    <div className={`user-card group relative rounded-xl p-5 shadow-sm border flex flex-col md:grid md:grid-cols-12 gap-4 items-center transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 hover:border-gray-300 dark:hover:border-gray-600 cursor-pointer bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 hover:bg-[#D9ECC8] dark:hover:bg-[#006655]/20
      ${isOpen ? 'z-50' : 'z-10'}
    `}>
      <div className="col-span-12 md:col-span-4 flex items-center w-full">
        ...
        <div className="ml-4 overflow-hidden">
          <div className="text-sm font-bold text-nordic dark:text-white truncate">{user.full_name || t.noName}</div>
          <div className="text-xs text-nordic/70 dark:text-gray-400 truncate">{user.email}</div>
          <div className="mt-1 text-[10px] px-2 py-0.5 inline-block bg-gray-50 dark:bg-white/10 rounded text-nordic/50 dark:text-gray-400 transition-colors group-hover:bg-white/50 dark:group-hover:bg-white/20">
      ...
      <div className="col-span-12 md:col-span-3 w-full flex items-center justify-between md:justify-start gap-4">
        {isAdmin ? (
          <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-[#19322F] text-white shadow-sm">
            {t.administrator}
          </span>
        ) : (
        ...
        <div className="flex items-center text-xs text-nordic/60 dark:text-gray-400">
          <span className="material-icons text-[14px] mr-1 text-mosque">check_circle</span> {t.active}
        </div>
      </div>
      
      <div className="col-span-12 md:col-span-3 w-full grid grid-cols-2 gap-4">
        <div>
          <div className="text-[10px] uppercase tracking-wider text-nordic/40">{t.properties}</div>
          <div className="text-sm font-semibold text-nordic dark:text-white">0</div>
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-wider text-nordic/40">{isAdmin ? 'Access Level' : t.salesYTD}</div>
      ...
      <div className="col-span-12 md:col-span-2 w-full flex justify-end relative" ref={dropdownRef}>
        <button 
          ...
          ${isAdmin 
            ...
            : 'border border-gray-200 dark:border-gray-600 bg-transparent text-nordic/70 dark:text-gray-300 hover:border-nordic hover:text-nordic dark:hover:text-white dark:hover:border-gray-400 hover:bg-white hover:shadow-sm group-hover:bg-white dark:group-hover:bg-gray-700'
          }`
        }>
          {isPending ? t.updating : t.changeRole} <span className="material-icons text-[16px] ml-2">{isOpen ? 'expand_less' : 'expand_more'}</span>
        </button>
          ...
          <div className="absolute top-full right-0 mt-2 w-48 rounded-lg shadow-dropdown bg-mosque ring-1 ring-black ring-opacity-5 focus:outline-none overflow-hidden z-[100] origin-top-right transition-all">
            <div aria-labelledby="options-menu" aria-orientation="vertical" className="py-1" role="menu">
              <button onClick={() => handleRoleChange('admin')} className="w-full text-left group flex items-center px-4 py-3 text-xs text-white/70 hover:bg-white/10 hover:text-white transition-colors cursor-pointer" role="menuitem">
                <span className={`material-icons text-sm mr-3 ${isAdmin ? 'text-white' : 'text-white/50 group-hover:text-white'}`}>shield</span> {t.administrator}
              </button>
              <button onClick={() => handleRoleChange('user')} className="w-full text-left group flex items-center px-4 py-3 text-xs text-white/70 hover:bg-white/10 hover:text-white transition-colors cursor-pointer" role="menuitem">
                <span className={`material-icons text-sm mr-3 ${!isAdmin ? 'text-white' : 'text-white/50 group-hover:text-white'}`}>person</span> {t.user}
              </button>
            </div>
          </div>
        )}
