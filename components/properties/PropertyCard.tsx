...
export const PropertyCard = ({ property, className = "" }: Props) => {
	return (
		<Link href={`/properties/${property.slug}`}
			className={`block bg-white dark:bg-white/5 rounded-xl overflow-hidden shadow-card hover:shadow-soft transition-all duration-300 group cursor-pointer h-full flex flex-col ${className}`}
		>
			...
			<div className="p-4 flex flex-col grow">
				...
				<div className="mt-auto flex items-center justify-between pt-3 border-t border-gray-100 dark:border-white/10">
					<div className="flex items-center gap-1 text-nordic-muted text-xs">
						<BedDouble
							className="w-3.5 h-3.5 text-mosque/80"
							strokeWidth={1.5}
						/>{" "}
						{property.beds}
					</div>
					<div className="flex items-center gap-1 text-nordic-muted text-xs">
						<Bath className="w-3.5 h-3.5 text-mosque/80" strokeWidth={1.5} />{" "}
						{property.baths}
					</div>
					<div className="flex items-center gap-1 text-nordic-muted text-xs">
						<Maximize
